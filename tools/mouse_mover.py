...
class MouseMover(VirtualBoxController):
    ...
    def mouse_clicker(self, x2, y2, buttonToleranceX, buttonToleranceY):
        ...
        xrand = int(np.random.normal(round(buttonToleranceX/2),round(buttonToleranceX/6), 1)[0])
        yrand = int(np.random.normal(round(buttonToleranceY/2),round(buttonToleranceY/6), 1)[0])
