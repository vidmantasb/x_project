...
class Decision(DecisionBase):
    ...
    def preflop_override(self, t, logger, h, p):
        if t.gameStage == GameStages.PreFlop.value:
            ...
            if found_card != '':
                ...
                if t.abs_equity>0.9 and t.allInCallButton_found:
                    self.decision = DecisionTypes.call
                    self.logger.info('Preflop calling activated from preflop table')
                elif rnd < call_probability and not t.allInCallButton_found:
                    self.decision = DecisionTypes.call
                    self.logger.info('Preflop calling activated from preflop table')

                elif rnd >= call_probability and rnd <= bet_probability + call_probability and not t.allInCallButton_found:
                ...
                if t.equity >= float(p.selected_strategy['alwaysCallEquity']):
                    self.decision = DecisionTypes.call
                    self.logger.info('Preflop folding, hands not found in preflop table. Montecarlo is used.')
                else:
                    self.decision = DecisionTypes.fold
            ...
            # absolute value decision
                ...
                if not (h.round_number > 0 and h.previous_decision == DecisionTypes.check_deception.value):
                    ...
                    if (self.finalBetLimit >= float(t.totalPotValue) / 2) \
                        ...
                        self.decision = DecisionTypes.bet3
                    # bet4
                    if t.allInCallButton_found == False and t.equity >= float(p.selected_strategy['betPotRiverEquity']) and \
                                    t.gameStage == GameStages.River.value and \
    ...
    def admin(self, t, p, h):
        ...
        if t.allInCallButton_found and self.decision != DecisionTypes.fold:
            self.decision = DecisionTypes.call
