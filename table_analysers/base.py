...
class Table(object):
    ...
    def load_templates(self, p):
        ...
        if self.tbl[0:2] == 'SN':
            ...
            template = Image.open(name)
            self.rightpot = cv2.cvtColor(np.array(template), cv2.COLOR_BGR2RGB)

            name = "pics/" + self.tbl[0:2] + "/left_pot_2.png"
            template = Image.open(name)
            self.leftpot = cv2.cvtColor(np.array(template), cv2.COLOR_BGR2RGB)

            name = "pics/" + self.tbl[0:2] + "/betbutton_1.png"
            template = Image.open(name)
            self.betbutton1 = cv2.cvtColor(np.array(template), cv2.COLOR_BGR2RGB)

            name = "pics/" + self.tbl[0:2] + "/notlostEverything.png"
            template = Image.open(name)
            self.notlostEverything = cv2.cvtColor(np.array(template), cv2.COLOR_BGR2RGB)
    ...
    def get_ocr_float(self, img_orig, name, force_method=0, binarize=False, replace_black_white=True):
        ...
        if replace_black_white:
            img_orig = replace_black_white_colors(img_orig, 150)
