...
class TableScreenBased(Table):
    def get_top_left_corner(self, p):
        ...
        count, points, bestfit, _ = self.find_template_on_screen(self.topLeftCorner, img, 0.05)
        try:
    ...
    def check_for_betbutton(self):
        ...
        if count == 0:
            count, points, bestfit, _ = self.find_template_on_screen(self.betbutton1, img, func_dict['tolerance'])
    ...
    def check_for_allincall(self):
        ...
        if count > 0:
            self.allInCallButton_found = True
            self.logger.debug("All in call button found")
        else:
            self.allInCallButton_found = False
    ...
    def get_my_cards(self, h):
        ...
        if len(self.mycards) == 2:
            self.my_cards_found = True
            self.logger.info("My cards: " + str(self.mycards))
            return True
        else:
            self.my_cards_found = False
    ...
    def get_other_player_names(self, p):
        if p.selected_strategy['gather_player_names'] == 1:
            ...
            self.other_players_names_found = True
            for i, fd in enumerate(func_dict):
                ...
                try:
                    ...
                    self.other_players_names_found = False
    ...
    def get_bot_pot(self, p):
        ...
        try:
            if self.tbl=='PS2':
                pil_image_1 = self.crop_image(self.entireScreenPIL, self.tlc[0] + fd[0], self.tlc[1] + fd[1] - 10, self.tlc[0] + fd[2],
                                           self.tlc[1] + fd[3] + 10)
                img = cv2.cvtColor(np.array(pil_image_1), cv2.COLOR_BGR2RGB)
                count_1, points_1, bestfit_1, _ = self.find_template_on_screen(self.rightpot, img, 0.01)
                pil_image = self.crop_image(self.entireScreenPIL, self.tlc[0] + fd[0], self.tlc[1] + fd[1],
                                           self.tlc[0] + fd[0] + points_1[0][0]+8,
                                           self.tlc[1] + fd[3])
                value = self.get_ocr_float(pil_image, str(inspect.stack()[0][3]), force_method=0)
            else:
    ...
    def get_dealer_position(self):
        ...
        try:
            self.dealer_position_found = True
            point = points[0]
        except:
            self.dealer_position_found = False
    ...
    def get_total_pot_value(self, h):
        ...
        if self.tbl == 'PS':
            ...
            if count:
                number_image = self.crop_image(pil_image,  left_points[0][0] + 7, 5, right_points[0][0]+4, pil_image.size[1]-5)
                value = self.get_ocr_float(number_image, str(inspect.stack()[0][3]), force_method=0) # force_method was 1 before
    ...
    def get_current_bet_value(self, p):
        ...
        if self.currentCallValue == '':
            ...
            if p.selected_strategy['pokerSite'][0:2] == "PS" and self.allInCallButton_found:
                self.currentCallValue = self.currentBetValue
                self.currentBetValue = 9999999
            try:
    ...
    def get_lost_everything(self, h, t, p, gui_signals):
        ...
        if self.tbl == 'PS2':
            ...
            pil_image = self.crop_image(self.entireScreenPIL, self.tlc[0] + func_dict['x1'],
                                        ...
                                        self.tlc[0] + func_dict['x2'], self.tlc[1] + func_dict['y2'])
            # Convert RGB to BGR
            img_1 = cv2.cvtColor(np.array(pil_image), cv2.COLOR_BGR2RGB)
            count_1, points_1, bestfit_1, minvalue_1 = self.find_template_on_screen(self.ImBack, img_1, func_dict['tolerance'])

            img_2 = cv2.cvtColor(np.array(self.entireScreenPIL), cv2.COLOR_BGR2RGB)
            count_2, points_2, bestfit_2, _ = self.find_template_on_screen(self.notlostEverything, img_2, 0.01)

            iam_back = count_1>0
            empty_center = count_2>0
            self.lost_everything_found = iam_back&(empty_center==False)
        else:
            func_dict = self.coo[inspect.stack()[0][3]][self.tbl]
            pil_image = self.crop_image(self.entireScreenPIL, self.tlc[0] + func_dict['x1'], self.tlc[1] + func_dict['y1'],
                                        self.tlc[0] + func_dict['x2'], self.tlc[1] + func_dict['y2'])
